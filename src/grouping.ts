 ...
 * Khi không có mã BN, dùng sourcePath (đường dẫn file quét ra record đầu tiên
 * của nhóm) làm định danh ổn định thay vì vị trí mảng — vị trí mảng dịch
 * chuyển mỗi khi 1 record khác bị xoá (GroupedReview -> removeRecord), khiến
 * override/kết quả AI đã tính gán nhầm sang record khác ở lần render sau.
 */
export function aggOverrideKey(
  g: Pick<PatientGroup, 'idValue' | 'indices' | 'records'>,
  fieldKey: string
): string {
  const groupId = g.idValue || `file:${g.records[0]?.sourcePath || g.indices[0]}`;
